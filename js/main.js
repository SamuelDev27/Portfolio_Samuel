...
filterBtnsContainer.addEventListener("click", (e)=>{
    ...
    !e.target.classList.contains("active")){
        ...
        document.querySelector(".filter-status p").innerHTML = `<p>Filtrando por <span>${e.target.innerHTML}</span></p>`;
        setTimeout(() => {
...
function filterItems(filterBtn){
    ...
    document.querySelectorAll(".portfolio-item").forEach((item)=>{
        ...
        if(category.indexOf(selectedCategory) !== -1 || selectedCategory === "todos"){
...
function portfolioItemDetails(){
    ...
    document.querySelector(".pp-counter").innerHTML = `${portfolioItemIndex+1} de ${portfolioItems.length}
